mergeData <- function() {
  ...
  merged_train <- merge(ma,t,by=Reduce(intersect,list(names(ma),names(t))),all.y=T)
  write.csv(merged_train,"processed/merged_train.csv",row.names=F)
  merged_valid <- merge(ma,v,by=Reduce(intersect,list(names(ma),names(v))),all.y=T)
  write.csv(merged_valid,"processed/merged_test.csv",row.names=F)
}